...
def always_true(index):
    return True

env = gym.make('LunarLander-v2')

env_draw = gym.make('LunarLander-v2')
env_draw = gym.wrappers.Monitor(env, 'results', force=True, video_callable=always_true)
...
def run():
    ...
    while 1:
        try:
            gen_best, best_genomes = pop.run(ec.evaluate_genomes, 5)
            ...
            for k in range(100):
                ...
                while 1:
                    ...
                    if k == 0:
                        observation, reward, done, info = env_draw.step(best_action)
                    else:
