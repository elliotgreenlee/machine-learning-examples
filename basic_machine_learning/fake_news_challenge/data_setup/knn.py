...
class KNN:
    ...
    def training_features_by_class(self):
        for sample in self.training_classes:
            self.training_features_in_class[sample] += 1

        print(self.training_features_in_class)

    '''knn'''
    def knn(self):
        classes = []
        for index, sample in enumerate(self.testing_features):
            type = self.decide_knn(sample)
            print(index, type)
            classes.append(type)
    ...
    def decide_knn(self, sample):
        ...
        for i in range(0, self.k):
            class_counts[sorted_training_classes[i]] += 1
